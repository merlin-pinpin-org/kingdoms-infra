...
- **Humans never check out, write code, or run scripts.** This platform is
  ...
  the GitHub web UI, performed by authorized humans. Runner and VPS
  installation is the one exception (a human runs the documented
  installer on the machine itself, following VPS-SETUP.md).
- **Ops manage environments and secrets in the GitHub web UI only**
  (environment protection, required reviewers, environment secrets). The
  agent never handles secret values; production deployments are gated to
  ops (Actions policy on the Deploy prod workflow + `prod` environment
  approval).
