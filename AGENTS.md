...
- **Never commit secrets** (tokens, passwords, API keys, private keys,
  ...
  host-provisioned `.env` files (never committed; `deploy/*/.env.example`
...
regenerated by **PR comment commands**: a write-access collaborator comments
`/roadmap` or `/dependencies` on a `kingdoms` PR and the `PR commands`
workflow commits the synced file to that PR branch (see
[kingdoms/docs/SKILLS/pr-commands.md](https://github.com/merlin-pinpin/kingdoms/blob/main/docs/SKILLS/pr-commands.md)).
Do not edit these files manually — change the issue state or its
`## Dependencies` section instead, then run the command on an open
`kingdoms` PR. Priorities (`priority/P0-P3` labels) come from critical-path
analysis maintained by `kingdoms/scripts/sync_dependencies.py` — do not set
them by hand unless the analysis is wrong.
